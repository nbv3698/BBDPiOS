			...
			$.ajax({
				...
				success : function(response) {
					...
					for(var number = 0; number < response.itemNameList.length; number++){
						var itemName = returnEscapeCharacter(response.itemNameList[number]);		//item Name//解析
		...
		function changeList(){
			...
			//得到存在local storage的patientID值並顯示出來
			var patientID = window.localStorage.getItem('login');
		...
		function printList(response){
			...
			if(response.itemIDList.length == 0)
				$("#display").append(
						"<div><center><br><p class='basicText'>無健康追蹤紀錄</p></center></div>");
			//顯示所有紀錄
			for(var i=0; i<response.itemIDList.length; i++){
				...
				for(var number=0; number<detailIDList.length; number++){
					var detailName = returnEscapeCharacter(detailNameList[number]);	//解析
					var detailValue = detailValueList[number];
					var detailUnit = returnEscapeCharacter(detailUnitList[number]);	//解析
