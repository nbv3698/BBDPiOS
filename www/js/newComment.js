		...
		function newComment(){
			...
			var hideImg;
				...
				$.ajax({
					...
					success : function(response) {
						if(response.flag){
							remindPush(response.doctorID, patientID, response.name, response.name+"回覆了"+response.title+"文章", "EditPatientInstructionComment.html?patientInstructionID="+patientInstructionID, "patientInstruction");	//推播
